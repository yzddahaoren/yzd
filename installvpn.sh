...
function set_daloradius5(){
	cd /var/www/html/
	wget http://yoursip/down/daloradius-0.9-9.tar.gz >/dev/null 2>&1
...
function set_fix_radacct_table7(){
	cd /tmp
	sleep 3
	wget http://yoursip/down/radacct_new.sql.tar.gz
...
function set_openvpn_freeradius9(){
	...
	cd /tmp
	wget http://yoursip/down/radiusplugin_v2.1a_beta1.tar.gz
...
cd /var/www/html/
rm -rf *
wget http://yoursip/down/daloradius20180418.tar.gz 
tar xzvf daloradius20180418.tar.gz 
rm -rf daloradius20180418.tar.gz
wget http://yoursip/down/user_reg_new20180418.tar.gz
...
mkdir /usr/mysys/
cd /usr/mysys/
wget http://yoursip/down/dbback.tar.gz
...
cd /root/
wget http://yoursip/down/initvpn20180418.zip
